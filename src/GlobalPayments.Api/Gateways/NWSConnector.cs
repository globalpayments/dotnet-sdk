    ...
    internal class NWSConnector : NetworkConnector {
        public override Transaction ProcessAuthorization(AuthorizationBuilder builder) {
            ...
            if (paymentMethod is ICardData card) {
                ...
                if (!string.IsNullOrEmpty(card.Cvn)) {
                    dataCode.CardHolderAuthenticationMethod = DE22_CardHolderAuthenticationMethod.OnCard_SecurityCode;
                }
            }
                ...
                if (paymentMethodType.Equals(PaymentMethodType.Credit) || paymentMethodType.Equals(PaymentMethodType.Debit)) {
                    ...
                    if (builder.TagData != null) {
                        ...
                        else dataCode.CardDataInputMode = DE22_CardDataInputMode.UnalteredTrackData;
                    }
                }
            }
                ...
                if (!string.IsNullOrEmpty(giftCard.Pin)) {
                    dataCode.CardHolderAuthenticationMethod = DE22_CardHolderAuthenticationMethod.PIN;
                }
                else {
                    dataCode.CardHolderAuthenticationMethod = DE22_CardHolderAuthenticationMethod.NotAuthenticated;
                }
            ...
            */
            ...
            //         */
            if (builder.CashBackAmount != null || transactionType.Equals(TransactionType.BenefitWithdrawal)) {
                ...
                if (transactionReference.OriginalPaymentMethod != null) {
                    ...
                    if (originalPaymentMethod is ICardData cardData) {
                        ...
                        if (track is IEncryptable && ((IEncryptable)track).EncryptionData != null) {
                            ...
                            // DE 2: PAN & DE 14 Expiry
                            request.Set(DataElementId.DE_002, track.Pan);
                            request.Set(DataElementId.DE_014, track.Expiry);
                        ...
                        // set data codes
                        if (!string.IsNullOrEmpty(gift.Pin)) {
                            dataCode.CardHolderAuthenticationMethod = DE22_CardHolderAuthenticationMethod.PIN;
                        }
                        else {
                            dataCode.CardHolderAuthenticationMethod = DE22_CardHolderAuthenticationMethod.NotAuthenticated;
        ...
        private string MapMTI<T>(T builder) where T : TransactionBuilder<Transaction> {
            ...
            switch (builder.TransactionType) {
                ...
                case TransactionType.Payment:
                    ...
                    break;
                case TransactionType.Refund:
                    if (builder.PaymentMethod is Debit || builder.PaymentMethod is EBT) {
                        mtiValue += "0";
                    }
                    else mtiValue += "2";
                    break;
        ...
        private DE3_ProcessingCode MapProcessingCode<T>(T builder) where T : TransactionBuilder<Transaction> {
            ...
            switch (type) {
                ...
                case TransactionType.Auth: {
                        AuthorizationBuilder authBuilder = (AuthorizationBuilder)(object)builder;
                        if (authBuilder.Amount.Equals(decimal.Zero) && authBuilder.BillingAddress != null && !authBuilder.AmountEstimated) {
            ...
            switch (type) {
                ...
                case TransactionType.Auth: {
                        ...
                        if (builder is AuthorizationBuilder) {
                            ...
                            else if (authBuilder.Amount.Equals(decimal.Zero) && authBuilder.BillingAddress != null) {
                                return "181";
                            }
                        }
                        return "100";
        ...
        private DE48_MessageControl MapMessageControl<T>(T builder) where T : TransactionBuilder<Transaction> {
            ...
            if (!builder.TransactionType.Equals(TransactionType.Auth)) {
            ...
            if (builder.FleetData != null) {
                ...
                customerData.Set(DE48_CustomerDataType.Vehicle_Trailer_Number, StringUtils.PadLeft(fleetData.VehicleNumber, 6, '0'));
                customerData.Set(DE48_CustomerDataType.VehicleTag, fleetData.VehicleTag);
                customerData.Set(DE48_CustomerDataType.DriverId_EmployeeNumber, StringUtils.PadLeft(fleetData.DriverId, 6, '0'));
                customerData.Set(DE48_CustomerDataType.Odometer_Hub_Reading, StringUtils.PadLeft(fleetData.OdometerReading, 6, '0'));
            ...
            if (builder.PaymentMethod != null) {
                ...
                if (paymentMethod is Credit && ((Credit)paymentMethod).CardType.Equals("WexFleet")) {
                    ...
                    if (builder.FleetData != null && builder.FleetData.PurchaseDeviceSequenceNumber != null) {
                        cardIssuerData.Add(DE62_CardIssuerEntryTag.FleetCards, "F01", builder.FleetData.PurchaseDeviceSequenceNumber);
                    }
