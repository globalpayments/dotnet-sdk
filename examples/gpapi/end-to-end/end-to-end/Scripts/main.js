...
async function start3DS(token) {
    ...
    try {
         versionCheckData = await checkVersion('/Check3dsVersion.ashx', {
            tokenResponse: token 
        });
    ...
    try {
        // 3DS2 Flow
        if (versionCheckData.messageVersion == '2') {
            console.log("3ds2 flow")
            authenticateData = await initiateAuthentication('/InitiateAuthentication.ashx', {
                challengeWindow: {
            ...
            if (authenticateData.result == "SUCCESS_AUTHENTICATED" && authenticateData.liabilityShift == 'YES') {
                ...
                form.setAttribute("action", "Authorization.ashx");
