...
namespace GlobalPayments.Api.Terminals.UPA
{
    public class BatchReportResponse : ITerminalReport {
                ...
                else
                {
                    ...
                    if (Status == "Success") {
                        ...
                        if (batchRecord != null) {
                            ...
                            if (Command == UpaTransType.GetBatchReport) {
                                MapBatchTransactions(batchRecord);
                            }
                            else if(Command == UpaTransType.GetBatchDetails) {
                                MapTransactionDetails(batchRecord);
                            }                                            
                        }
                    }
                    else { 
        ...
        private void MapTransactionDetails(JsonDoc batchRecord) {           
            if(batchRecord.Has("batchDetailRecords") && BatchRecord.TransactionDetails == null) {
                BatchRecord.TransactionDetails = new List<TransactionSummary>();
            }
            DateTime defaultDateTime;
            decimal defaultDecimal;
            foreach (var transaction in batchRecord.GetEnumerator("batchDetailRecords")) {
                var transactionSummary = new TransactionSummary();
                transactionSummary.TransactionDate = DateTime.TryParse(transaction.GetValue<string>("transactionTime"), out defaultDateTime) ? defaultDateTime : default(DateTime?);
                transactionSummary.AuthCode = transaction.GetValue<string>("approvalCode");
                transactionSummary.AuthorizedAmount = decimal.TryParse(transaction.GetValue<string>("authorizedAmount"), out defaultDecimal) ? defaultDecimal : default(decimal?);
                transactionSummary.CardEntryMethod = transaction.GetValue<string>("cardAcquisition");
                transactionSummary.CardSwiped = transaction.GetValue<string>("cardSwiped");
                transactionSummary.CardType = transaction.GetValue<string>("cardType");
                transactionSummary.MaskedCardNumber = transaction.GetValue<string>("maskedPAN");
                transactionSummary.ReferenceNumber = transaction.GetValue<string>("referenceNumber");
                transactionSummary.IssuerTransactionId = transaction.GetValue<string>("gatewayTxnId");
                transactionSummary.ClerkId = transaction.GetValue<string>("clerkId") ?? null;
                transactionSummary.Amount = decimal.TryParse(transaction.GetValue<string>("requestedAmount"), out defaultDecimal) ? defaultDecimal : default(decimal?);
                transactionSummary.GatewayResponseCode = transaction.GetValue<string>("responseCode");
                transactionSummary.GatewayResponseMessage = transaction.GetValue<string>("responseText");
                transactionSummary.TransactionStatus = transaction.GetValue<string>("transactionStatus");
                transactionSummary.TransactionType = transaction.GetValue<string>("transactionType");
                transactionSummary.GratuityAmount = decimal.TryParse(transaction.GetValue<string>("tipAmount"), out defaultDecimal) ? defaultDecimal : default(decimal?);
                transactionSummary.SettlementAmount = decimal.TryParse(transaction.GetValue<string>("settleAmount"), out defaultDecimal) ? defaultDecimal : default(decimal?);
                transactionSummary.TaxAmount = decimal.TryParse(transaction.GetValue<string>("taxAmount"), out defaultDecimal) ? defaultDecimal : default(decimal?);                
                BatchRecord.TransactionDetails.Add(transactionSummary);
            }
        }

        private void MapBatchTransactions(JsonDoc batchRecord) {
            if (batchRecord.Has("batchTransactions") && BatchRecord.BatchTransactions == null) {
                BatchRecord.BatchTransactions = new List<BatchTransactionResponse>();
            }
            DateTime defaultDateTime;
            decimal defaultDecimal;
